...
const createSnacks = async (snack) => {
  try {
    const createSnack = await db.one(
      "INSERT INTO snacks (name, fiber, protein, added_sugar, is_healthy,amount, image) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *",
      [
        snack.name,
        snack.fiber,
        snack.protein,
...
const updateSnacks = async (id, snack) => {
  ...
  try {
    const updateSnack = await db.one(
      "UPDATE snacks SET (name=$1, fiber=$2, protein=$3, added_sugar=$4, is_healthy=$5, amount=$6, image=$7) WHERE id=$8 RETURNING *",
      [
        snack.name,
        snack.fiber,
        snack.protein,
